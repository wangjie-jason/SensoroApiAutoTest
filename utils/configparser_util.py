...
class ConfigParserUtil:
    ...
    def __init__(self, dir_file_name):
        """
        :param dir_file_name: 项目下文件所在目录名及文件名，如：config/lins_environment.ini
        """
        # path = os.path.dirname(os.path.abspath(__file__))
        # file_path = os.path.join(path, '../config', file_name)
...
if __name__ == '__main__':
    config = ConfigParserUtil('config/lins_environment.ini').get_config()
    ...
    # 写入配置
    ConfigParserUtil('config/lins_environment.ini').write_config(config)
